...
from modules.dbutils import list_dir
...
class classify:
    ...
    def Train(self,save_path="Classifier/model.ckpt"):
        ...
        cross_entropy = tf.reduce_mean(cross_entropy) + 0.05 * sum(reg_losses)
        ...
        for i in range(50000):
            ...
            if i % 10000 == 0:
                test_ = mnist.test.next_batch(10000)
                test_accuracy = self.sess_classifier.run(accuracy, feed_dict = {self.Xmn: test_[0], self.Ymn: test_[1], self.keep_prob:1.0})
                print('[classify_mnist_v2.py -- Train] (Iter %d) Test accuracy %g' %(i, test_accuracy))
            self.sess_classifier.run(train_step, feed_dict={self.Xmn: batch[0], self.Ymn: batch[1], self.keep_prob: 0.3})
    ...
    def TrainwithoutSave(self):
        mnist = input_data.read_data_sets("/home/mangroup/Documents/Code/Generative_Adversarial_Network/gaan/gaan_mnist/mnist/", one_hot=True)
        self.y_conv, self.keep_prob = self.Build_model()
        with tf.name_scope('loss'):
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=self.Ymn,
                                                                logits=self.y_conv)
    ...
    def Evaluate_Labels(self, source, model_path="Classifier/model.ckpt", ext='jpg'):
        ...
        self.load_images(source, ext)

        Curr_Preds = sess_classifier.run(self.y_conv, feed_dict={self.Xmn: self.ImagesR, self.keep_prob: 1.0})
        Curr_Labels = np.argmax(Curr_Preds, 1)

        Curr_Preds2 = sess_classifier.run(self.y_conv, feed_dict={self.Xmn: self.ImagesG, self.keep_prob: 1.0})
        Curr_Labels2 = np.argmax(Curr_Preds2, 1)

        Curr_Preds3 = sess_classifier.run(self.y_conv, feed_dict={self.Xmn: self.ImagesB, self.keep_prob: 1.0})
        Curr_Labels3 = np.argmax(Curr_Preds3, 1)

        return Curr_Preds, Curr_Labels, Curr_Preds2, Curr_Labels2,Curr_Preds3, Curr_Labels3
    ...
    def Compute_Activations(self, sess, source, model_path="Classifier/model.ckpt", ext='jpg', restore=True):
        ...
        self.load_gray_images(source, ext)

        activations = sess.run(self.h_pool2, feed_dict={self.Xmn: self.Images, self.keep_prob: 1.0})
    ...
    def load_images(self, source, ext):
        ...
        print('[classify_mnist_v2.py -- load_images] load test images from %s' % (source))
        print('[classify_mnist_v2.py -- load_images] total images: %d' % (nb_imgs))
    ...
    def load_gray_images(self, source, ext):
        im_list  = list_dir(source, ext)
        nb_imgs  = len(im_list)
        self.Images=np.zeros((nb_imgs,784))
        for k in range(nb_imgs):
            img = imread(im_list[k])
            self.Images[k,:] = np.reshape(img,(1,784)) / 255.0
