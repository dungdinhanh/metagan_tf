...
class MSDistGAN(object):
    ...
    def __init__(self, model='metagan', \
        ...
        if self.verbose == True:
            print(
                '[msdistgan.py -- __init__] model = %s, lr = %s, beta1 = %f, beta2 = %f, decay_step = %d, decay_rate = %f' % (
                self.model, self.lr, self.beta1, self.beta2, self.decay_step, self.decay_rate))
        ...
        if self.verbose == True:
            ...
            print('[msdistgan.py -- __init__] ef_dim = %d, gf_dim = %d, df_dim = %d' % (
            self.ef_dim, self.gf_dim, self.df_dim))
        ...
        if self.verbose == True:
            print('[msdistgan.py -- __init__] ss_task = %d, lambda_d = %f, lambda_g = %f' % (
            self.ss_task, self.lambda_d, self.lambda_g))
        ...
        if self.verbose == True:
            print('[msdistgan.py -- __init__] lambda_p = %f, lambda_r = %f, lambda_w = %f' % (
            self.lambda_p, self.lambda_r, self.lambda_w))

        self.nb_test_real = nb_test_real
        self.nb_test_fake = nb_test_fake

        if self.verbose == True:
            print('[msdistgan.py -- __init__] FID: nb_test_real = %d, nb_test_fake = %d' % (
            self.nb_test_real, self.nb_test_fake))
    ...
    def create_model(self):
        ...
        with tf.variable_scope('discriminator'):
            ...
            if self.ss_task == 1 or self.ss_task == 2:
                self.d_real_sigmoid, self.d_real_logit, self.f_real, _ = self.D(self.X, self.data_shape,
                                                                                dim=self.df_dim, ss_task=self.ss_task,
                                                                                reuse=False)
                self.d_fake_sigmoid, self.d_fake_logit, self.f_fake, _ = self.D(self.X_f, self.data_shape,
                                                                                dim=self.df_dim, ss_task=self.ss_task,
                                                                                reuse=True)
                self.d_recon_sigmoid, self.d_recon_logit, self.f_recon, _ = self.D(self.X_r, self.data_shape,
                                                                                   dim=self.df_dim,
                                                                                   ss_task=self.ss_task, reuse=True)
            else:  # original D loss
                self.d_real_sigmoid, self.d_real_logit, self.f_real = self.D(self.X, self.data_shape, dim=self.df_dim,
                                                                             ss_task=self.ss_task, reuse=False)
