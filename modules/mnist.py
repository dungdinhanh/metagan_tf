...
def load_mnist(source, percent):
    ...
    n = int(N * percent / 100)
    data   = data[permutation[0:n],:]
    labels = labels[permutation[0:n]]
    print('[mnist.py] data shape: {}'.format(np.shape(data)))
    print('[mnist.py] labels shape: {}'.format(np.shape(labels)))
    return data, labels

def load_small_mnist(source, N = 100):
    mnist  = input_data.read_data_sets(source)
    data_full   = mnist.train.images
    label_full  = mnist.train.labels
    m = data_full.shape[0]
    permutation = list(np.random.permutation(m))
    data  = data_full[permutation[0:N],:]
    label = label_full[permutation[0:N]]
    label = np.reshape(label,(-1,1))
    print('[mnist.py] data shape: {}'.format(np.shape(data)))
    print('[mnist.py] labels shape: {}'.format(np.shape(label)))
    return data, label
